...
f = open("C:/Users/94883/Desktop/WS/ws2_django_demo/static/big_index_dictionary_tfidf_2.txt", 'r+')
dic = eval(f.read()) #global index dictionary
f.close()
set_of_word = set(re.findall(r'\w+',open('C:/Users/94883/Desktop/WS/ws2_django_demo/static/big.txt').read().lower())) #a text with lots of word

def homepage(request):
    context = dict()
    if request.method == "POST": #IF it is post, should jump to the result.html
        form = SearchForm(request.POST)
        if form.is_valid():
            post_data = form.cleaned_data['search']
        context['form'] = form
        return result(request)#go to the search page
    else :#If it is get, just stay
        form = SearchForm()
        context['form'] = form
